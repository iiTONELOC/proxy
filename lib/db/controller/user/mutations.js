...
const userMutations = {
    async createNewUser(parent, args, context) {
        const { geo } = context;
        console.log(`CREATE NEW USER MUTATION`, context)
        let LOC_ID, STATUS_ID, SERVER_ID, PROFILE_ID, UPDATED_S_ID;
        const data = { latitude: geo.latitude, longitude: geo.longitude, city: geo.city, state: `${geo.country, geo.region}` } = await IpLocation.user(args, geo);
        try {
            const userData = {
                username,
                email,
