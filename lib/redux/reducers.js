import { updateFriendImages } from '../idb';
import {
...
// update idb here as well
// currently only update friends profile pictures
    ...
    if (typeof window === Object || typeof window === 'object') {
        switch (action?.type) {
            case _REDUX_SET_USER_DATA:
                // update idb here
                action?.me?.friends?.length > 0 ?
                    updateFriendImages(action.me.friends) : null;
                return {
                    ...state,
                    me: action.me,
                }
            ...
            case _REDUX_UPDATE_FRIENDS_LIST:
                updateFriendImages(action.friends)
                return {
                    ...state,
