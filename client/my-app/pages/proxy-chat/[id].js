...
import Test from '../../components/UsersInRange'
    ...
    return (
        <Authorization>
            <div>
                ...
                <ResponsiveLayout viewData={{ Test: { Element: Test, props: userData.usersInRange } }} />
            </div>
        </Authorization>
    )
}
...
export async function getServerSideProps(req) {
    ...
    const { data, error } = await serverClient.query({ query: SERVER_SIDE_FETCH_USER, variables: { user: id }, fetchPolicy: "network-only" });
    if (error) {
